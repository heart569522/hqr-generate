...
use hqr_generate::{
    generate_qr_bitmap, generate_qr_modules, render_png, render_png_modules, render_svg,
    render_svg_modules,
};
...
fn bench_generate(c: &mut Criterion) {
    ...
    for (label, text) in [("short", SHORT), ("url", URL), ("long", LONG)] {
        g.bench_function(label, |b| {
            b.iter(|| generate_qr_modules(black_box(text), 320, 4, 2).unwrap())
        });
    }
    g.finish();
}
...
fn bench_svg(c: &mut Criterion) {
    let m_short = generate_qr_modules(SHORT, 320, 4, 2).unwrap();
    let m_long = generate_qr_modules(LONG, 320, 4, 2).unwrap();
    let b_short = generate_qr_bitmap(SHORT, 320, 4, 2).unwrap();
    let b_long = generate_qr_bitmap(LONG, 320, 4, 2).unwrap();
...
fn bench_end_to_end(c: &mut Criterion) {
    ...
    g.bench_function("modules→1bit-png (url)", |b| {
        b.iter(|| {
            let m = generate_qr_modules(black_box(URL), 320, 4, 2).unwrap();
    ...
    g.bench_function("bitmap→8bit-png/legacy (url)", |b| {
        b.iter(|| {
            let bm = generate_qr_bitmap(black_box(URL), 320, 4, 2).unwrap();
...
fn bench_output_size(c: &mut Criterion) {
    ...
    let m = generate_qr_modules(URL, 320, 4, 2).unwrap();
    let bm = generate_qr_bitmap(URL, 320, 4, 2).unwrap();
